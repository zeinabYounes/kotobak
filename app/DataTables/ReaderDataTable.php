...
class ReaderDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
      ...
      ->addColumn('action', function ($query) use($design) {
            $model_edit = $design->make_modal($this->editRow(route("admin.readers.store"),$query,"Update"),"Reader","Edit" ,$query->id);
            $model_delete = $design->make_modal($this->deleteRow(route("admin.readers.destroy",$query->id)),"Reader","Delete",$query->id);
    ...
    {
        $design = new DesignButton;
        $model_create = $design->make_modal($this->editRow(route("admin.readers.store"),null,"Create"),"Reader","Create",0);
    ...
    public function editRow($route,$qry,$action ){
      $body_pass = '
            <div >
               ...
               <input type="email"  value ="'.($qry?$qry->email:'').'" class="form-control" id="nm" placeholder="Enter Reader Email" name="email">
            </div>
            <div >
               <label for="" class="form-label"> Password :</label>
               <input type="password"  value ="" class="form-control" id="nm" placeholder="Enter Reader Password" name="password">
            </div>
            <div class="">
      ...
      <input type="hidden" name="user_id" value="'.($qry?$qry->id:'').'">
                ...
                <div class="row">
                  <div >
                     ...
                     <input type="text"  value ="'.($qry?$qry->name:'').'" class="form-control" id="nm" placeholder="Enter Reader Name" name="name">
                  </div>
