...
class SectionsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->editColumn('count_books', function ($query) {
               return $query->loadCount('books')->books_count;
          })
        ->editColumn('count_shelves', function ($query) {
               return $query->loadCount('shelves')->shelves_count;
          })
        ->addIndexColumn()
        ->addColumn('action', function ($query) use($design) {
              $model_edit = $design->make_modal($this->editRow(route("admin.sections.store"),$query,"Update"),"Section","Edit" ,$query->id);
              $model_delete = $design->make_modal($this->deleteRow(route("admin.sections.destroy",$query->id)),"Section","Delete",$query->id);
            return '<div class="btn-group ">'.$design->make_show(route("admin.sections.show",$query->id))." ".$design->make_edit_modal($query->id)." ".$design->make_delete_modal($query->id).'</div>' .$model_edit.$model_delete;
          })
        ->rawColumns([
            'action'
         ]);
    ...
    {
        $design = new DesignButton;
        $model_create = $design->make_modal($this->editRow(route("admin.sections.store"),null,"Create"),"Section","Create",0);
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('s_name')->addClass('font-weight-bold  font-italic small '),
            Column::make('count_shelves')->title('count shelves')->addClass('font-weight-bold  font-italic small '),
      ...
      <input type="hidden" name="section_id" value="'.$id.'">
                ...
                <div class="row">
                  <div >
                     ...
                     <input type="text"  value ="'.$name.'" class="form-control" id="nm" placeholder="Enter Section Name" name="s_name">
                  </div>
                  <br>
